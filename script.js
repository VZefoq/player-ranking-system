        ...
        // Load players from localStorage on page load, migrate old data if needed
        let playersData = JSON.parse(localStorage.getItem('playersData')) || [];

        // Migrate old data to include timestamps
        playersData = playersData.map(player => {
            if (!player.timestamp) {
                player.timestamp = Date.now(); // Add current timestamp for existing players
            }
            return player;
        });
        ...
        function saveEdit(index) {
            ...
            playersData[index].timestamp = Date.now(); // Update timestamp when edited
            saveToLocalStorage();
            updateLeaderboard();
        }
            ...
            reader.onload = function(e) {
                try {
                    ...
                    importData.players.forEach(importedPlayer => {
                        // Ensure imported player has required fields
